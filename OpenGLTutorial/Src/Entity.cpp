...
namespace Entity {
	...
	*エンティティバッファを作成する
	*
	*@param	maxEntityCount		扱えるエンティティを最大数
	*@param	ubSizePerEntity		エンティティごとのUniform Bufferのバイト数
	*@param	bindingPoint		エンティティ用ＵＢＯのバインディングポイント
	...
	*/
	BufferPtr Buffer::Create(size_t maxEntityCount, GLsizeiptr ubSizePerEntity, int bindingPoint, const char* ubName) {
		...
		if (!p->ubo || !p->buffer) {
			...
			std::cerr << "WARNING in Entity::Buffer::Create: バッファuboの作成に失敗" << std::endl;
			return{};
		}
	...
	*@param	matObj		Projection行列
	*/
	void Buffer::Update(double delta, const glm::mat4& matView, const glm::mat4& matPoj) {
		...
		for (itrUpdate = activeList.next; itrUpdate != &activeList;
			...
			if (e.updateFunc) {
				...
				e.updateFunc(e, p + e.uboOffset, delta, matView, matPoj);
			}
		}
	...
	*アクティブなエンティティを描画する
	*
	*@param	meshBuffer	描画に称するメッシュバッファへのポインタ
	*/
