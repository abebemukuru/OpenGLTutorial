...
reinterpret_cast<GLvoid*>(offsetof(cls,mbr)))
	...
	*/
	struct TemporaryMaterial {
		...
		std::vector<uint_fast32_t> indexBuffer;
	...
	*FBXデータを仮データに変換する
	*
	*@param	fbxNode	変換対象のFBXnodeへのポインタ
	*
	*@retval	true	変換成功
	*@retval	false	変換失敗
	*/
	...
	*/
	void Mesh::Draw(const BufferPtr& buffer)const {
		...
		if (!buffer) {
			std::cerr << "buffer is not!" << std::endl;
			return;
		}
	...
	*@param	iboSize	バッファに格納可能なインデックス数
	*/
		...
		for (TemporaryMesh& e : loader.meshList) {
			for (TemporaryMaterial& material : e.materialList) {
				...
				if (vboEnd + verticesBytes >= vboSize) {
					std::cerr << "WARNING: VBO サイズが不足しています(" <<
				...
				if (iboEnd + indicesBytes >= iboSize) {
					std::cerr << "WARNING: IBO サイズが不足しています(" <<
