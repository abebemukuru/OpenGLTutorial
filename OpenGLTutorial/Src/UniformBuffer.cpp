...
*@retval	false	転送失敗
*
*offsetとsizeの両方が０の場合、バッファサイズ=UBOサイズとして転送される
*/
bool UniformBuffer::BuffferSubData(const GLvoid* data, GLintptr offset, GLsizeiptr size) {

	if (offset + size > this->size) {

		std::cerr << "ERROR(" << name << "): 転送範囲がバッファサイズを超えています(buffer=" << this->size << " offset" << offset <<
			"size=" << size << ")" << std::endl;
