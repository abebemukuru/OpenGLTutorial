...
namespace Shader {
	...
	*/
	ProgramPtr Program::Create(const char* vsFilename, const char* fsFilename) {
		...
		//頂点シェーダファイル名の末尾から.vertを取り除いたものをプログラム名とする
		p->name = vsFilename;
	...
	*/
	...
	/**
	*テクスチャをテクスチャ・ユニットに割り当てる
	*
	*@param	uint	割り当て先のテクスチャ・イメージ・ユニット番号（GL_TEXTURE0~）
	...
	*
	*@param	type	シェーダの種類
	*@param	string	シェーダコードのポインタ
	*
	*@return	作成したシェーダオブジェクト
	*/
	GLuint ConplileShader(GLenum type, const GLchar* string) {
	...
	*プログラムオブジェクトを作成する
	*
	*@param	vsCode	頂点シェーダコードのポインタ
	*@param	fsCode	フラグ面とシェーダコードのポインタ
	*
	*@return	作成したプログラムオブジェクト
	*/
	GLuint CreateShaderProgram(const GLchar* vsCode, const GLchar* fsCode) {

		GLuint vs = ConplileShader(GL_VERTEX_SHADER, vsCode);
		GLuint fs = ConplileShader(GL_FRAGMENT_SHADER, fsCode);
		if (!vs || !fs) {
			return 0;
		}
	...
	*/
	bool ReadFile(const char* filename, std::vector<char>& buf) {
		...
		if (readSize != st.st_size) return false;
